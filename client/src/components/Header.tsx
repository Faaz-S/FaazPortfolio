...
export default function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = ['home', 'about', 'why-me', 'education', 'experience', 'projects', 'skills', 'contact'];
      ...
      <nav className="container mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <h1 className="text-2xl font-bold neon-text">Faaz Sherwani</h1>
            <span className="hidden md:block text-gray-400 text-sm font-mono">
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`nav-link text-sm font-medium ${
                  activeSection === item.id ? 'text-neon-green' : ''
                }`}
              >
                {item.label}
              </button>
